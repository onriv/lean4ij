...
- [ ] infoview toolwindow in swing
  ...
  - [x] popup
  - [x] pop up style, fonts, clickable links, etc
  - [x] color
  - [x] make the editor singleton
- [x] mathlib4 seems always failed starting the language server
...
- [x] distinguish source in .lake as library rather than source
...
- [x] jcef infoview style adjust
- [ ] unify jcef/browser/external/vscode infoview font name...
- [x] jcef infoview popup link should be opened in external web browser
- [ ] setting page
  - [x] added a setting page
  - [x] add some other settings
  - [x] lsp autocompletion disable setting
  - [ ] setting for getAllMessages
- [x] line comment and block comment
- [x] weird, is it just me or any other reason making only word autocompletion not working? In comment, it works but in normal pos it does not. It seems it's superseded by 
...
– [x] TODO weird brackets does not complete
  - [here](https://github.com/intellij-rust/intellij-rust/issues/1076) maybe related
– [x] maybe it's still better define some lang-like feature using parser/lexer, although it cannot be full parsed, but for the level like textmate it should be OK
– [ ] is it possible do something like pygments/ctags/gtags completion?
– [ ] option to skip library or backend files
– [x] error seems quite delay vanish... it shows errors event it has been fixed.
- [x] the internal infoview some case also delay (especially using ideavim one does not move caret)
– [x] comment auto comment like /-- trigger block comment
– [x] bock/line comment command
– [x] impl simp? which replace the code
...
- [x] internal infoview when expanding all messages it seems jumping
...
- [x] maybe internal infoview should be soft wrap. If the text is too long, currently only making the toolwindow wider we can see it
- [x] internal infoview update is not instant (It may require some update of cursor)
- [x] if the project contains some failed to build event, the internal infoview totally failed to update
